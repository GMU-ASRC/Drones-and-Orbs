...
# -- search --
SEARCH_YAW_DPS    = 10.0    # spin rate while looking (keep <= ~50 for 10fps cam)

# -- detection freshness --
LOST_GRACE_S      = 2.5     # keep chasing last bearing this long
FRESH_S           = 0.6     # cam ~5.6 fps
...
LOG_EVERY_S       = 0.5     # detection log throttle
PX_TO_M           = 358.0   # d(m) ~= PX_TO_M / radius(px); from 2.5 m calib
# ================================================================
...
def main():
    ...
    try:
        ...
        last_yaw_cmd = 0.0                 # keep turning toward last sighting
        last_log = 0.0
        ...
        while True:
            ...
            _now = time.monotonic()
            if _now - last_log >= LOG_EVERY_S:
                if fresh:
                    _d = PX_TO_M / det.radius if det.radius > 0 else 0.0
                    print(f"[det] {state:8s} SEE  "
                          f"ang=({det.ang_x:+5.1f},{det.ang_y:+5.1f})deg  "
                          f"r={det.radius:5.1f}px  area={det.area:6d}  "
                          f"d~{_d:4.1f}m", flush=True)
                else:
                    _gone = _now - last_seen if last_seen else 0.0
                    print(f"[det] {state:8s} ---  no target for {_gone:4.1f}s",
                          flush=True)
                last_log = _now
            ...
            # ====================== APPROACH ======================
                ...
                if not fresh:
                    ...
                    if time.monotonic() - last_seen > LOST_GRACE_S:
                        ...
                        # keep turning toward where it was last seen
                        drone.move_body(0.0, 0.0, last_yaw_cmd)
                else:
                    ...
                    elif det.radius >= R_CLUMP:
                        # centered AND close enough -> clump
                        drone.hold()
                        goto('CLUMPED')
                    else:
