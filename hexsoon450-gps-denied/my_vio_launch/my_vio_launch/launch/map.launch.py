...
def generate_launch_description():
    ...
    # ── RealSense D435i ────────────────────────────────────────────────────────
    # unite_imu_method:
    #   '1' = copy (copies accel ts to gyro) — causes timestamp collisions in
    #         newer realsense2_camera driver versions → rgbd_odometry drops IMU
    #   '2' = linear_interpolation — recommended by RTAB-Map for D435i; avoids
    #         the duplicate-timestamp issue that triggers "Dropping imu data!"
    ...
    PythonLaunchDescriptionSource(
        ...
        launch_arguments={
            ...
            'accel_fps':        '200', # (an apt upgrade of rtabmap_examples can change these)
            'localization': 'false',
            'delete_db_on_start': 'false',
            'database_path': '~/.ros/rtabmap.db',
            'rtabmap_args': '--Mem/IncrementalMemory true --Mem/InitWMWithAllNodes true',
        }.items()
    )
